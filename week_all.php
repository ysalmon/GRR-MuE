							...
							if ($d[$cday]["id_room"][$i]==$row['2'])
							{
								...
								if ($acces_fiche_reservation)
								{
									if (Settings::get("display_level_view_entry") == 0)
									{
										...
										$id = $d[$cday]["id"][$i];
										echo '<a title="'.htmlspecialchars($d[$cday]["who"][$i]).'" class="lienModal" onclick="requestModal('.$id.','.$cday.','.$cmonth.','.$cyear.',\''.$currentPage.'\',readDataModal);" data-toggle="modal" data-target="#myModal">';
					...
					else
					{
						...
						if ((($authGetUserLevel > 1) || ($auth_visiteur == 1)) 
							...
							if ($enable_periods == 'y')
								...
								echo '<a href="edit_entry.php?room=',$row["2"],'&amp;hour=',$hour,'&amp;minute=00&amp;year=',$cyear,'&amp;month=',$cmonth,'&amp;day=',$cday,'&amp;page=week_all" title="',get_vocab("cliquez_pour_effectuer_une_reservation"),'" onclick="jQuery(\'div.loader\').show();"><span class="glyphicon glyphicon-plus"></span></a>',PHP_EOL;;
						}
						else
							echo ' '.PHP_EOL;
					}
